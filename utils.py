...
def sc_projection(chromatin_profile_preds, clustervfeat):
    return np.dot(chromatin_profile_preds, clustervfeat.T) / np.linalg.norm(clustervfeat, axis=1)


def sc_hnorm_varianteffect(
    chromatin_profile_ref, chromatin_profile_alt, clustervfeat, histone_inds
):
    ...
    chromatin_profile_ref_adjust[:, histone_inds] = (
        ...
        * (
            ...
            / np.sum(chromatin_profile_ref[:, histone_inds], axis=1)
        )[:, None]
    )
    ...
    chromatin_profile_alt_adjust[:, histone_inds] = (
        ...
        * (
            ...
            / np.sum(chromatin_profile_alt[:, histone_inds], axis=1)
        )[:, None]
    )
...
def write_to_tsv(
    ...
    sorted_maxsc_df = pd.DataFrame(
        sorted_sc_abs_diff,  # dataframe
    ...
    sei_df = pd.concat([sorted_maxsc_df, sorted_rowlabels, sorted_profiles_df], axis=1)
    sc_df = pd.concat([sorted_maxsc_df, sorted_rowlabels, sorted_sc_df], axis=1)
